<template>
  ...
  export default {
    ...
    methods: {
      ...
      const tenantStartDates = this.tenants.map(tenant => tenant.start_date);
      const tenantEndDates = this.tenants.map(tenant => tenant.end_date);

      new Chart(chartCanvas, {
        type: 'bar',
        data: {
          labels: tenantStartDates.map((_, index) => `Tenant ${index + 1}`),
          datasets: [
            {
              label: 'Start Date',
              ...
              borderWidth: 1,
              data: tenantStartDates.map(date => new Date(date).getTime()),
            },
            {
              label: 'End Date',
              ...
              borderWidth: 1,
              data: tenantEndDates.map(date => new Date(date).getTime()),
            },
          ],
        },
        options: {
